...
class telemetry_primaryheader_adder(gr.basic_block):
    ...
    def __init__(self, transfer_frame_version_number, spacecraft_id, virtual_channel_id, ocf_flag, master_channel_frame_count,
        ...
        gr.basic_block.__init__(self,
            ...
            in_sig=[np.int16],
            out_sig=[ np.complex64, np.int16, np.int16 , np.int6])
    ...
    def work(self):
        ...
        def calculateFinalHeader(self):
            finalHeader = array.array('B', telemetry.PrimaryHeader.build(
        ...
        header = calculateFinalHeader()
        size = calculateSize()
        packet = sendPacket(array.array('B'),packet[:])
        msg = handlemsg(msg_pmt)

        return header , size , packet , msg
